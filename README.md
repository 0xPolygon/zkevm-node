...
- State: Responsible for managing the state data (batches, blocks, transactions, ...) that is stored on the `state DB`. It also handles the integration with the `executor` and the `Merkletree` service
